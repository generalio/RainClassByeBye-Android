...
import com.rainclass.core.model.OptionsEntity
import com.rainclass.core.model.ProblemsEntity
import com.rainclass.core.model.SolverAnswer
...
object AnswerParser {
    ...
    fun parseStructuredAnswer(raw: String, expectedProblemId: Long): SolverAnswer? {
        for (candidate in answerCandidates(raw)) {
            parseStructuredCandidate(candidate, expectedProblemId)?.let { return it }
        }
        return null
    }
    ...
    private fun answerCandidates(raw: String): List<String> {
        val candidates = mutableListOf<String>()
        fun add(v: String) {
            val trimmed = v.trim()
            if (trimmed.isNotEmpty() && trimmed !in candidates) candidates.add(trimmed)
        }
    ...
    private fun parseStructuredCandidate(content: String, expectedProblemId: Long): SolverAnswer? {
        return try {
    ...
    private fun extractNarrativeAnswer(raw: String): String {
        for (line in reverseNonEmptyLines(raw)) {
            ...
            var candidate = line
            for (sep in listOf("：", ":", "=")) {
                val idx = candidate.lastIndexOf(sep)
                if (idx >= 0 && idx + sep.length < candidate.length) {
                    candidate = candidate.substring(idx + sep.length)
                    break
                }
            }
            val weiIdx = candidate.lastIndexOf("为")
            if (weiIdx >= 0 && weiIdx + "为".length < candidate.length) {
                candidate = candidate.substring(weiIdx + "为".length)
            }
            val result = sanitizeNarrative(candidate)
