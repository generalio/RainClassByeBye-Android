...
import com.rainclass.core.network.llm.LlmCompletionRequest
import com.rainclass.core.network.llm.LlmMessage
...
class LLMSolver(
  ...
  private val systemPrompt = listOf(
    ...
    "你必须只返回一个 JSON 对象，不能返回 Markdown、代码块、" +
      "解释、题目复述、LaTeX 推导或任何前后缀。",
    ...
    "多选题返回多个选项 key。",
    "填空题或主观题只返回最终答案，返回字符串数组，" +
      "每个字符串对应一个答案，不要输出解题过程。",
  ...
  suspend fun solve(problem: ProblemsEntity): Triple<SolverAnswer, String, Exception?> {
    try {
      ...
      val repairMessages = messages + listOf(
        LlmMessage(role = "assistant", text = raw),
        LlmMessage(
          role = "user",
          text =
            "你上一个回答${if (raw.isBlank()) "为空字符串" else "不是纯 JSON"}，" +
              "无法解析为合法 JSON。" +
              "请基于同一道题重新输出一个 JSON 对象。" +
              """只允许输出形如 {"problem_id":${problem.problemId},"result":["A"]} """ +
              "的内容，不要解释，不要代码块，不要额外文本。"
        )
      )

      val repairResponse = llmClient.complete(
        settings = settings,
        request = LlmCompletionRequest(
      ...
      val repairedRaw = repairResponse.text
  ...
  private fun buildMessages(problem: ProblemsEntity): List<LlmMessage> {
    ...
    return listOf(
      ...
      LlmMessage(
        role = "user",
        text = "只允许返回一个 JSON 对象，且第一个字符必须是 {，" +
          "最后一个字符必须是 }。不要解释，不要复述题目，" +
          "不要输出代码块。题目 JSON 如下：$questionJson",
        imageUrls = imageUrls
      )
    )
  }
